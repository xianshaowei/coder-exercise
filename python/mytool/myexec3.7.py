...
class myconn:
    ...
    def nc_port_test(self, host, que):
        ...
        if not is_valid_ip(host) and re.match('^cld-', host) == None:
            ...
            return 1
        port_test_command = "nc  -G 3 -w 1  " + host + " DEFAULTPORT >/dev/null  2>&1"
    ...
    # 基本方法
    def root_to_run_oneip_multiline(self, host, command, que, port=DEFAULTPORT,):
    ...
    def root_to_run_oneip_oneline(self, host, command, que):
        ...
        for i in range(1, len(res_list) - 1):
            alist.append(res_list[i])

        for i in range(len(alist)):
            if i == 0:
                # print("\033[1;32;40m%s\033[0m") % alist[i],
                print(alist[i], end='' + " -----> " + cmd)
            elif i == len(alist) - 1:
                print(alist[i])
            else:
                print(alist[i], end='')
    ...
    def root_to_do_ipfile_mulline(self, ipfile, command, que):
        '''对IP列表的IP地址，依次进行执行命令（单进程单线程） '''
        if not os.access(ipfile, os.F_OK):
            print('The iplist  " %s " is not exist.' % ipfile)
            return 1
        iplist = []
        with open(ipfile, "r") as fp:
        ...
        for i in iplist:
            ...
            port = i[1]
            print(ip,port)
            p = Process(target=self.root_to_run_oneip_multiline, args=(ip, command, que, port))
            p.daemon = True
            process_list.append(p)
        ...
        # task_list = []
        # for ip in iplist:
        #     gtask = gevent.spawn(self.root_to_run_oneip_multiline, ip, command, que)
        #     task_list.append(gtask)
        #
        # for g in task_list:
        #     g.join()
        #
        ...
        process_list = []
        for ip in iplist:
            pool.apply_async(self.root_to_run_oneip_multiline, args=(ip, command, que))
        pool.close()
        for ip in iplist:
            time.sleep(0.1)
            pool.join()
    ...
    def upload_file(self, que, host, uploadfile, username='xianwei01', remote_path=remote_inventory,
                    local_path=file_inventory, port=DEFAULTPORT):
        ...
        try:
            ...
            print(
                "\033[1;31;40m%s \033[0m USERNAME or PASSWORD error or UPLOADFILE does not exist,pls check\t") % error_print
            return 1
    ...
    def multi_process_to_upload_iplist(self, ip_string, uploadfile, que):
        ...
        for ip in iplist:
            p = Process(target=self.upload_file,
                        args=(que, ip, uploadfile, 'xianwei01', remote_inventory, file_inventory, DEFAULTPORT))
            p.daemon = True
            process_list.append(p)
...
def main():
    ...
    # 指定参数
    par.add_argument('--port', '-p', type=int, help="special a port")
    # 指定参数
    ...
    elif args.ip and args.port and args.command :
        # 单个主机执行命令
        conn.root_to_run_oneip_multiline(args.ip, args.command, main_que, args.port)
